...
import {
	...
	Sender,
	Response
} from '@didvault/sdk/src/core/transport';
import { marshal, unmarshal } from '@didvault/sdk/src/core/';
		...
		request
			.on('data', (chunk) => {
				...
				if (size > maxSize) {
					...
					request.destroy(
						new Err(ErrorCode.MAX_REQUEST_SIZE_EXCEEDED)
					);
				}
				body.push(chunk);
			})
			...
			switch (httpReq.method) {
				...
				case 'POST':
					try {
						...
						const ipAddress =
							httpReq.headers['x-forwarded-for'] ||
							httpReq.socket?.remoteAddress;
						req.ipAddress = Array.isArray(ipAddress)
							? ipAddress[0]
							: ipAddress;
						// const location =
						// 	req.ipAddress && (await getLocation(req.ipAddress));
			...
			(res) => {
				...
				res.on('end', () => {
					if (res.statusCode === 200) {
						...
						reject(
							`${res.statusCode} ${res.statusMessage} - Message:\n${resBody}`
						);
					}
				});
