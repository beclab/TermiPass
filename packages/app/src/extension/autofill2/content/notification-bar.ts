...
import { NotificationBarIframeInitData } from '../bar/abstractions/notification-bar';
 ...
 */
let notificationBarIframe: HTMLIFrameElement | null = null;
...
async function loadNotificationBar() {
	...
	let domObservationCollectTimeoutId: number | null = null;
	let collectPageDetailsTimeoutId: number | null = null;
	let handlePageChangeTimeoutId: number | null = null;
	...
	const changePasswordButtonContainsNames = new Set([
		'pass',
		'change',
		'contras',
		'senha'
	...
	const enableAddedLoginPrompt =
		(await sendExtensionMessage('bgGetEnableAddedLoginPrompt')) || true;
			...
			observer = new MutationObserver((mutations: MutationRecord[]) => {
				...
				for (let i = 0; i < mutations.length; i++) {
					...
					for (let j = 0; j < mutation.addedNodes.length; j++) {
						...
						const tagName =
							addedNode.tagName != null
								? addedNode.tagName.toLowerCase()
								: null;
						...
						if (
							tagName != null &&
							tagName === 'form' &&
							(addedNode.dataset == null ||
								!addedNode.dataset.bitwardenWatching)
						) {
						...
						const forms = addedNode.querySelectorAll(
							'form:not([data-bitwarden-watching])'
						);
				...
				if (doCollectPageDetails) {
					...
					domObservationCollectTimeoutId = window.setTimeout(
						collectPageDetails,
						1000
					);
				}
	...
	function collectPageDetails() {
		void sendExtensionMessage('bgCollectPageDetails', {
	...
	function watchForms(forms: FormData[]) {
		...
		forms.forEach((f: FormData) => {
			...
			const formId: string | null = f.form != null ? f.form.htmlID : null;
			...
			if (formEl == null) {
				...
				const index = parseInt(f.form.opid.split('__')[2], undefined);
	...
	function locateFields(watchedForm: WatchedForm) {
		...
		watchedForm.usernameEl = locateField(
			watchedForm.formEl,
			watchedForm.data.username,
			inputs
		);
		...
		if (watchedForm.usernameEl != null && watchedForm.data.password != null) {
			...
			watchedForm.data.passwords.forEach((passwordData: AutofillField) => {
				...
				const passwordEl = locatePassword(
					watchedForm.formEl,
					passwordData,
					inputs,
					false
	...
	function formSubmitted(e: Event) {
		...
		if (e.type === 'click') {
			...
			if (form == null && formOpId != null) {
				form =
					watchedForms.find((wf) => (wf.formEl as any).opid === formOpId)
						?.formEl || null;
			}
		} else {
		...
		for (let i = 0; i < watchedForms.length; i++) {
			...
			if (
				...
				const userNamePopulated =
					login.username != null && login.username !== '';
				const passwordPopulated =
			...
			if (enableChangedPasswordPrompt && watchedForms[i].passwordEls != null) {
				...
				const passwords: string[] =
					watchedForms[i].passwordEls == null
						? []
						: watchedForms[i]
								.passwordEls!.filter(
									(el: HTMLInputElement) => el.value != null && el.value !== ''
								)
								.map((el: HTMLInputElement) => el.value);

				let curPass: string | null = null;
				let newPass: string | null = null;
				let newPassOnly = false;

				if (
					watchedForms[i].passwordEls?.length === 3 &&
					passwords.length === 3
				) {
					...
					watchedForms[i].passwordEls?.length === 2 &&
					passwords.length === 2
				) {
					...
					if (passwords[0] === passwords[1]) {
						...
						const buttonText = getButtonText(
							getSubmitButton(form, changePasswordButtonNames)
						);
						if (buttonText) {
				...
				if (
					(newPass != null && curPass != null) ||
					(newPassOnly && newPass != null)
				) {
	...
	function getSubmitButton(
		...
		if (submitButton == null && wrappingElIsForm) {
			...
			submitButton = wrappingEl.querySelector(
				'button:not([type]):not([data-bitwarden-clicked])'
			);
			if (submitButton != null) {
		...
		if (submitButton == null) {
			...
			let typelessButton: HTMLElement | null = null;
	 ...
	 */
	function getButtonText(button: HTMLElement | null) {
		if (!button) {
			return null;
		}
	...
	function closeExistingAndOpenBar(type: string, typeData: any) {
		...
		const notificationBarUrl = 'www/bar.html';
	...
	function openBar(
		...
		if (notificationBarIframe && notificationBarIframe.contentWindow) {
			(notificationBarIframe.contentWindow.location as any) = barPageUrl;
		}
	}

	function setupInitNotificationBarMessageListener(
	...
	function handleExtensionDisconnection() {
		closeBar(false);
		if (domObservationCollectTimeoutId)
			clearTimeout(domObservationCollectTimeoutId);
		if (collectPageDetailsTimeoutId) clearTimeout(collectPageDetailsTimeoutId);
		if (handlePageChangeTimeoutId) clearTimeout(handlePageChangeTimeoutId);
		observer?.disconnect();
		observer = null;
