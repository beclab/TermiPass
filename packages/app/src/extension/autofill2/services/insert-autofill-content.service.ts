...
class InsertAutofillContentService
	implements InsertAutofillContentServiceInterface
{
	private readonly domElementVisibilityService: DomElementVisibilityService;
	private readonly collectAutofillContentService: CollectAutofillContentService;
	private readonly autofillInsertActions: AutofillInsertActions = {
		fill_by_opid: ({ opid, value }) =>
			this.handleFillFieldByOpidAction(opid, value),
	...
	private userCancelledInsecureUrlAutofill(
		...
		if (
			!savedUrls?.some((url) =>
				url.startsWith(`https://${globalThis.location.hostname}`)
			) ||
	 ...
	 * @private
	 */
	private userCancelledUntrustedIframeAutofill(
		...
		const confirmationWarning = [
			...
			chrome.i18n.getMessage('autofillIframeWarningTip', [
				globalThis.location.hostname
			])
		].join('\n\n');
	 ...
	 * @private
	 */
	private insertValueIntoField(
		element: FormFieldElement | undefined,
		value: string
	) {
		...
		if (!elementIsFillableFormField(element)) {
			this.handleInsertValueAndTriggerSimulatedEvents(
				element,
	...
	private triggerFillAnimationOnElement(element: FormFieldElement): void {
		const skipAnimatingElement =
			...
			!new Set(['email', 'text', 'password', 'number', 'tel', 'url']).has(
				element?.type
			);

		if (
			this.domElementVisibilityService.isElementHiddenByCss(element) ||
			skipAnimatingElement
		) {
			return;
		}
	...
	private triggerFocusOnElement(
		...
		let initialValue = '';
		if (shouldResetValue && element && 'value' in element) {
			initialValue = String((element as any).value);
		}

		element?.focus();

		if (initialValue && element && 'value' in element) {
			(element as any).value = initialValue;
		}
	}
	 ...
	 * @private
	 */
	private simulateUserKeyboardEventInteractions(
		element: FormFieldElement
	): void {
		const simulatedKeyboardEvents = [
			EVENTS.KEYDOWN,
			EVENTS.KEYPRESS,
			EVENTS.KEYUP
		];
		for (let index = 0; index < simulatedKeyboardEvents.length; index++) {
			element.dispatchEvent(
				new KeyboardEvent(simulatedKeyboardEvents[index], { bubbles: true })
			);
		}
	}
