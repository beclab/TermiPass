...
class CollectAutofillContentService
	implements CollectAutofillContentServiceInterface
{
		...
		void sendExtensionMessage(
			...
			(useTreeWalkerStrategyFlag) =>
				(this.useTreeWalkerStrategyFlagSet =
					!!useTreeWalkerStrategyFlag?.result)
		);
	}
	...
	async getPageDetails(): Promise<AutofillPageDetails> {
		...
		const { formElements, formFieldElements } =
		...
		const autofillFieldsData: AutofillField[] = (
			await this.buildAutofillFieldsData(
	 ...
	 */
	getAutofillFieldElementByOpid(opid: string): FormFieldElement | undefined {
		const cachedFormFieldElements = Array.from(
			this.autofillFieldElements.keys()
		);
		...
		const fieldElementsWithOpid = formFieldElements.filter(
			(fieldElement) =>
				(fieldElement as ElementWithOpId<FormFieldElement>).opid === opid
	...
	deepQueryElements<T>(
		...
		const shadowRoots = this.recursivelyQueryShadowRoots(
			root,
		...
		for (let index = 0; index < shadowRoots.length; index++) {
			...
			elements = elements.concat(
				this.queryElements<T>(shadowRoot, queryString)
			);
		}
	...
	private queryElements<T>(
		...
		return Array.from(root.querySelectorAll(queryString)) as any as T[];
	}
	...
	private recursivelyQueryShadowRoots(
		...
		for (let index = 0; index < shadowRoots.length; index++) {
			...
			shadowRoots = shadowRoots.concat(
				this.recursivelyQueryShadowRoots(shadowRoot)
			);
	...
	private queryShadowRoots(
		...
		for (let index = 0; index < potentialShadowRoots.length; index++) {
			...
			if (shadowRoot && !ignoreIds.find((e) => e == shadowRoot.host.id)) {
	...
	private sortAutofillFieldElementsMap() {
		...
		this.autofillFieldElements = new Map(
			[...this.autofillFieldElements].sort(
	 ...
	 * @private
	 */
	private buildAutofillFormsData(
		formElements: Node[]
	): Record<string, AutofillForm> {
		for (let index = 0; index < formElements.length; index++) {
			const formElement = formElements[
				index
			] as ElementWithOpId<HTMLFormElement>;
	 ...
	 * @private
	 */
	private getFormActionAttribute(
		element: ElementWithOpId<HTMLFormElement>
	): string {
		return new URL(
			this.getPropertyOrAttribute(element, 'action') as any,
	...
	private async buildAutofillFieldsData(
		...
		const autofillFieldDataPromises = autofillFieldElements.map(
			this.buildAutofillFieldItem as any
		) as any;
	...
	private getAutofillFieldElements(
		...
		const numberUnimportantFieldsToInclude =
			fieldsLimit - priorityFormFields.length;
	...
	private buildAutofillFieldItem = async (
		...
		const autofillFieldBase = {
			...
			viewable: await this.domElementVisibilityService.isFormFieldViewable(
				element
			),
		...
		if (elementType !== 'hidden') {
			autofillFieldLabels = {
				'label-tag': this.createAutofillFieldLabelTag(
					element as FillableFormFieldElement
				),
		...
		const fieldFormElement = (
			element as ElementWithOpId<FillableFormFieldElement>
		).form;
		const autofillField = {
			...autofillFieldBase,
			...autofillFieldLabels,
			...
			selectInfo: elementIsSelectElement(element)
				...
				: null,
			form: fieldFormElement
				? this.getPropertyOrAttribute(fieldFormElement, 'opid')
				: null,
		...
		this.cacheAutofillFieldElement(index, element, autofillField as any);
		void this.autofillOverlayContentService?.setupAutofillOverlayListenerOnField(
			element,
			autofillField as any
		);
		return autofillField as any;
	};
	 ...
	 * @private
	 */
	private getAutoCompleteAttribute(
		element: ElementWithOpId<FormFieldElement>
	): string {
	...
	private getAttributeLowerCase(
		...
		return (
			this.getPropertyOrAttribute(element, attributeName)?.toLowerCase() || ''
		);
	}
	 ...
	 * @private
	 */
	private createAutofillFieldLabelTag(
		...
		const labelElements: NodeListOf<HTMLLabelElement> | null =
			this.queryElementLabels(element);
		if (labelElements) {
			for (
				let labelIndex = 0;
				labelIndex < labelElements!.length;
				labelIndex++
			) {
		...
		if (
			...
			elementIsDescriptionDetailsElement(element.parentElement || undefined) &&
			elementIsDescriptionTermElement(
				element.parentElement?.previousElementSibling || undefined
			)
		) {
			if (element.parentElement?.previousElementSibling)
				labelElementsSet.add(
					element.parentElement.previousElementSibling as any
				);
		}
	 ...
	 * @private
	 */
	private createLabelElementsTag = (
		labelElementsSet: Set<HTMLElement>
	): string => {
	 ...
	 * @private
	 */
	private getAutofillFieldMaxLength(
		...
		const elementMaxLength =
			elementHasMaxLengthProperty && element.maxLength > -1
				? element.maxLength
				: 999;

		return elementHasMaxLengthProperty
			? Math.min(elementMaxLength, 999)
			: undefined;
	}
	...
	private createAutofillFieldLeftLabel(element: FormFieldElement): string {
		const labelTextContent: string[] =
			this.recursivelyGetTextFromPreviousSiblings(element);
	 ...
	 * @private
	 */
	private createAutofillFieldTopLabel(
		element: FormFieldElement
	): string | null {
		...
		return parentSiblingTableRowElement?.cells?.length > tableDataElementIndex
			? this.getTextContentFromElement(
					parentSiblingTableRowElement.cells[tableDataElementIndex]
			  )
			: null;
	}
	 ...
	 * @private
	 */
	private isNewSectionElement(
		currentElement: HTMLElement | Node | null
	): boolean {
		if (!currentElement) {
			return true;
		}
	 ...
	 * @private
	 */
	private trimAndRemoveNonPrintableText(textContent: string | null): string {
		return (textContent || '')
	 ...
	 * @private
	 */
	private recursivelyGetTextFromPreviousSiblings(
		element: Node | HTMLElement | null
	): string[] {
	 ...
	 * @private
	 */
	private getPropertyOrAttribute(
		element: HTMLElement,
		attributeName: string
	): string | undefined {
	 ...
	 * @private
	 */
	private getSelectElementOptions(element: HTMLSelectElement): {
		options: (string | null)[][];
	} {
	...
	private isNodeFormFieldElement(node: Node): boolean {
		...
		const nodeIsValidInputElement =
			nodeTagName === 'input' &&
			!this.ignoredInputTypes.has((node as HTMLInputElement).type);
		if (nodeIsValidInputElement && !nodeHasBwIgnoreAttribute) {
			return true;
		}

		return (
			this.nonInputFormFieldTags.has(nodeTagName) && !nodeHasBwIgnoreAttribute
		);
	}
	...
	private processMutations = () => {
		for (
			let queueIndex = 0;
			queueIndex < this.mutationsQueue.length;
			queueIndex++
		) {
	 ...
	 */
	private isAutofillElementNodeMutated(
		nodes: NodeList,
		...
		for (let index = 0; index < nodes.length; index++) {
			...
			const autofillElements = this.useTreeWalkerStrategyFlagSet
				...
				: this.deepQueryElements<HTMLElement>(
						node,
						`form, ${this.formFieldQueryString}`,
						true
				  );
		...
		if (isRemovingNodes) {
			for (
				let elementIndex = 0;
				elementIndex < mutatedElements.length;
				elementIndex++
			) {
	...
	private setupOverlayListenersOnMutatedElements(mutatedElements: Node[]) {
		for (
			...
			if (
				...
				this.autofillFieldElements.get(
					node as ElementWithOpId<FormFieldElement>
				)
			) {
				continue;
			}
			...
			globalThis.requestIdleCallback(
				...
				() =>
					void this.buildAutofillFieldItem(
						node as ElementWithOpId<FormFieldElement>,
						-1
					),
				{ timeout: 1000 }
	...
	private deleteCachedAutofillElement(
		...
		if (
			elementIsFormElement(element) &&
			this.autofillFormElements.has(element)
		) {
	...
	private updateAutofillFormElementData(
		...
		const updateActions: Record<string, CallableFunction> = {
			action: () =>
				(dataTarget.htmlAction = this.getFormActionAttribute(element)),
	...
	private updateAutofillFieldElementData(
		...
		const updateActions: Record<string, CallableFunction> = {
			maxlength: () =>
				(dataTarget.maxLength = this.getAutofillFieldMaxLength(element)),
			id: () => updateAttribute('htmlID'),
			name: () => updateAttribute('htmlName'),
			...
			tagname: () =>
				(dataTarget.tagName = this.getAttributeLowerCase(element, 'tagName')),
			type: () =>
				(dataTarget.type = this.getAttributeLowerCase(element, 'type')),
			value: () => (dataTarget.value = this.getElementValue(element)),
			checked: () =>
				(dataTarget.checked = this.getAttributeBoolean(element, 'checked')),
			disabled: () =>
				(dataTarget.disabled = this.getAttributeBoolean(element, 'disabled')),
			readonly: () =>
				(dataTarget.readonly = this.getAttributeBoolean(element, 'readonly')),
			autocomplete: () =>
	...
	private setupIntersectionObserver() {
		this.intersectionObserver = new IntersectionObserver(
			this.handleFormElementIntersection,
			{
				root: null,
				rootMargin: '0px',
				threshold: 1.0
	 ...
	 */
	private handleFormElementIntersection = async (
		entries: IntersectionObserverEntry[]
	) => {
		for (let entryIndex = 0; entryIndex < entries.length; entryIndex++) {
			const entry = entries[entryIndex];
			const formFieldElement =
				entry.target as ElementWithOpId<FormFieldElement>;
			...
			const isViewable =
				await this.domElementVisibilityService.isFormFieldViewable(
					formFieldElement
				);
			if (!isViewable) {
				continue;
			...
			const cachedAutofillFieldElement =
				this.autofillFieldElements.get(formFieldElement);
	...
	isPasswordFieldWithinDocument(): boolean {
		...
		return Boolean(
			this.deepQueryElements(document, `input[type="password"]`)?.length
		);
	}
}
