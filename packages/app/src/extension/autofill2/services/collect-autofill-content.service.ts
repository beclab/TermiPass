	...
	implements CollectAutofillContentServiceInterface
{
	...
	private getFormActionAttribute(
		...
		return new URL(
			this.getPropertyOrAttribute(element, 'action') || '',
	...
	private async buildAutofillFieldsData(
		...
		const autofillFieldDataPromises = autofillFieldElements.map((e, index) => {
			return this.buildAutofillFieldItem(e as any, index);
		});
	...
	private getAutoCompleteAttribute(
		...
		return autoCompleteType !== 'off' ? autoCompleteType : undefined;
	}
	...
	private createAutofillFieldLabelTag(
		...
		const labelElements: NodeListOf<HTMLLabelElement> | undefined =
		...
		while (currentElement && currentElement !== document.documentElement) {
			...
			currentElement =
				currentElement.parentElement?.closest('label') || undefined;
		}
	...
	private getPropertyOrAttribute(
		...
		if (attributeName in element) {
			return (element as FormElementWithAttribute)[attributeName] || undefined;
		}
	...
	private isNodeFormFieldElement(node: Node): boolean {
		...
		return (
			['textarea', 'select'].includes(nodeTagName) && !nodeHasBwIgnoreAttribute
		);
	}
	...
	private handleMutationObserverMutation = (mutations: MutationRecord[]) => {
		...
		for (
			...
			mutationsIndex < mutations.length;
			mutationsIndex++
		) {
