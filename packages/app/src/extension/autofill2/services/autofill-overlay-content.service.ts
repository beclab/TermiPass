	...
	implements AutofillOverlayContentServiceInterface
{
	...
	private readonly generateRandomCustomElementName =
		generateRandomCustomElementName;
	...
	private mostRecentlyFocusedField: ElementWithOpId<FormFieldElement> | null;
	...
	private autofillFieldKeywordsMap: WeakMap<AutofillField, string> =
		new WeakMap();
	...
	async setupAutofillOverlayListenerOnField(
		...
		if (
			this.isIgnoredField(autofillFieldData) ||
	...
	openAutofillOverlay(options: OpenAutofillOverlayOptions = {}) {
		const { isFocusingFieldElement, isOpeningFullOverlay, authStatus } =
			options;
		if (!this.mostRecentlyFocusedField) {
			return;
		}
		...
		if (
			...
			!this.recentlyFocusedFieldIsCurrentlyFocused()
		) {
	...
	redirectOverlayFocusOut(direction: string) {
		...
		if (!this.focusableElements.length) {
			this.focusableElements = this.findTabs(globalThis.document.body, {
				getShadowRoot: true
			});
		}
		...
		const redirectFocusElement =
			this.focusableElements[focusedElementIndex + indexOffset];
	...
	private setupFormFieldElementEventListeners(
		...
		formFieldElement.addEventListener(
			EVENTS.KEYUP,
			this.handleFormFieldKeyupEvent as any
		);
		formFieldElement.addEventListener(
			EVENTS.INPUT,
	 ...
	 */
	private removeCachedFormFieldEventListeners(
		formFieldElement: ElementWithOpId<FormFieldElement>
	) {
		const handlers = [EVENTS.INPUT, EVENTS.CLICK, EVENTS.FOCUS];
		for (let index = 0; index < handlers.length; index++) {
			const event = handlers[index];
			const memoIndex = this.getFormFieldHandlerMemoIndex(
				formFieldElement,
				event
			);
	 ...
	 */
	private useEventHandlersMemo = (
		eventHandler: EventListener,
		memoIndex: string
	) => {
		return (
			this.eventHandlersMemo[memoIndex] ||
			(this.eventHandlersMemo[memoIndex] = eventHandler)
		);
	};
	 ...
	 */
	private handleFormFieldClickEvent = (
		formFieldElement: ElementWithOpId<FormFieldElement>
	) => {
		return this.useEventHandlersMemo(
	 ...
	 * @param formFieldElement - The form field element that triggered the click event.
	 */
	private async triggerFormFieldClickedAction(
		formFieldElement: ElementWithOpId<FormFieldElement>
	) {
	 ...
	 */
	private handleFormFieldFocusEvent = (
		formFieldElement: ElementWithOpId<FormFieldElement>
	) => {
		return this.useEventHandlersMemo(
	 ...
	 * @param formFieldElement - The form field element that triggered the focus event.
	 */
	private async triggerFormFieldFocusedAction(
		formFieldElement: ElementWithOpId<FormFieldElement>
	) {
		...
		await this.updateMostRecentlyFocusedField(formFieldElement);
		const formElementHasValue = Boolean(
	...
	private recentlyFocusedFieldIsCurrentlyFocused() {
		return (
			this.getRootNodeActiveElement(this.mostRecentlyFocusedField) ===
	...
	private async updateMostRecentlyFocusedField(
		...
		if (formFieldElement) {
			const { paddingRight, paddingLeft } =
				globalThis.getComputedStyle(formFieldElement);
	...
	private async getMostRecentlyFocusedFieldRects(
		...
		const focusedFieldRects =
			await this.getBoundingClientRectFromIntersectionObserver(
				formFieldElement
			);
	...
	private async getBoundingClientRectFromIntersectionObserver(
		...
		return new Promise((resolve) => {
			const intersectionObserver = new IntersectionObserver(
				(entries) => {
					let fieldBoundingClientRects: any = entries[0].boundingClientRect;
					if (
						!fieldBoundingClientRects?.width ||
						!fieldBoundingClientRects.height
					) {
	...
	private createAutofillOverlayButton() {
		...
		this.overlayButtonElement =
			window.document.createElement(customElementName);
	}
	...
	private createAutofillOverlayList() {
		...
		this.overlayListElement =
			globalThis.document.createElement(customElementName);
	}
	...
	private async getAutofillOverlayVisibility() {
		const overlayVisibility = await this.sendExtensionMessage(
			'getAutofillOverlayVisibility'
		);
		this.autofillOverlayVisibility =
			overlayVisibility || AutofillOverlayVisibility.OnFieldFocus;
	}
	...
	private triggerOverlayRepositionUpdates = async () => {
		...
		this.toggleOverlayHidden(false);
		this.clearUserInteractionEventTimeout();
		const top = this.focusedFieldData.focusedFieldRects?.top || 0;
		if (top > 0 && top < globalThis.innerHeight) {
			return;
		}
	...
	private observeCustomElements() {
		...
		if (this.overlayListElement) {
			this.overlayElementsMutationObserver?.observe(this.overlayListElement, {
				attributes: true
			});
		}
	}
	...
	private observeBodyElement() {
		this.bodyElementMutationObserver?.observe(globalThis.document.body, {
			childList: true
		});
	}
	 ...
	 */
	private handleOverlayElementMutationObserverUpdate = (
		...
		for (
			let recordIndex = 0;
			recordIndex < mutationRecord.length;
			recordIndex++
		) {
	...
	private handleBodyElementMutationObserverUpdate = () => {
		...
		const secondToLastChildIsOverlayButton =
			secondToLastChild === this.overlayButtonElement;
	 ...
	 */
	private getRootNodeActiveElement(element: Element | null): Element | null {
