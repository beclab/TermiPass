...
class DomElementVisibilityService
	implements domElementVisibilityServiceInterface
{
	...
	async isFormFieldViewable(element: FormFieldElement): Promise<boolean> {
		...
		return this.formFieldIsNotHiddenBehindAnotherElement(
			element,
	...
	private isElementNotVisible(element: HTMLElement): boolean {
		return new Set(['hidden', 'collapse']).has(
			this.getElementStyle(element, 'visibility')
		);
	}
	...
	private isElementOutsideViewportBounds(
		...
		const elementTopOffset =
			elementBoundingClientRect.top - documentElement.clientTop;
		const elementLeftOffset =
			elementBoundingClientRect.left - documentElement.clientLeft;

		const isElementSizeInsufficient =
			elementBoundingClientRect.width < 10 ||
			elementBoundingClientRect.height < 10;
		const isElementOverflowingLeftViewport = elementLeftOffset < 0;
		const isElementOverflowingRightViewport =
			elementLeftOffset + elementBoundingClientRect.width >
			documentElementWidth;
		const isElementOverflowingTopViewport = elementTopOffset < 0;
		const isElementOverflowingBottomViewport =
			elementTopOffset + elementBoundingClientRect.height >
	...
	private formFieldIsNotHiddenBehindAnotherElement(
		...
		const rootElement =
			elementRootNode instanceof ShadowRoot
				? elementRootNode
		...
		const targetElementLabelsSet = new Set(
			(targetElement as FillableFormFieldElement).labels
		);
