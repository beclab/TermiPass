import { EVENTS, setElementStyles } from '../../utils';
import {
...
class AutofillOverlayIframeService
	implements AutofillOverlayIframeServiceInterface
{
	...
	private updateIframePosition(position?: Partial<CSSStyleDeclaration>) {
		...
		setTimeout(
			() => this.updateElementStyles(this.iframe, { opacity: '1' }),
			0
		);
		this.announceAriaAlert();
	}
	...
	private handleElementAttributeMutation(element: HTMLElement) {
		...
		for (
			...
			const defaultIframeAttribute =
				this.defaultIframeAttributes[attribute.name];
