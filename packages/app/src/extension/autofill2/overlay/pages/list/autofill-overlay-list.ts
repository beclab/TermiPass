...
class AutofillOverlayList extends AutofillOverlayPageElement {
	...
	private readonly overlayListWindowMessageHandlers: OverlayListWindowMessageHandlers =
		{
			...
			updateOverlayListCiphers: ({ message }) =>
				this.updateListItems(message.ciphers),
	...
	private async initAutofillOverlayList({
		...
		const linkElement = this.initOverlayPage(
			'button',
			styleSheetUrl,
			translations
		);
		...
		this.overlayListContainer.classList.add(
		...
		if (authStatus === AuthenticationStatus.Unlocked && ciphers) {
	...
	private buildNoResultsOverlayList() {
		...
		this.overlayListContainer.append(
			noItemsMessage,
			overlayListButtonContainer
		);
	}
	...
	private loadPageOfCiphers() {
		...
		for (
			let cipherIndex = this.currentCipherIndex;
			cipherIndex < lastIndex;
			cipherIndex++
		) {
			this.ciphersList.appendChild(
				this.buildOverlayActionsListItem(this.ciphers[cipherIndex])
			);
	...
	private handleCiphersListScrollEvent = () => {
		...
		this.cipherListScrollDebounceTimeout = setTimeout(
	...
	private handleDebouncedScrollEvent = () => {
		...
		if (
			...
			this.ciphersList.clientHeight - 300
		) {
			this.loadPageOfCiphers();
		}
	};
	...
	private buildFillCipherElement(cipher: OverlayCipherData) {
		...
		fillCipherElement.addEventListener(
			EVENTS.KEYUP,
			this.handleFillCipherKeyUpEvent
		);
	 ...
	 */
	private buildCipherNameElement(
		cipher: OverlayCipherData
	 ...
	 */
	private buildCipherUserLoginElement(
		cipher: OverlayCipherData
	): HTMLSpanElement | null {
	...
	private focusNextListItem(currentListItem: HTMLElement) {
		...
		const nextSibling = nextListItem?.querySelector(
			'.fill-cipher-button'
		) as HTMLElement;
		if (nextSibling) {
			nextSibling.focus();
		...
		const firstListItem = currentListItem.parentElement
			?.firstChild as HTMLElement;
		const firstSibling = firstListItem?.querySelector(
			'.fill-cipher-button'
		) as HTMLElement;
		firstSibling?.focus();
	}
	...
	private focusPreviousListItem(currentListItem: HTMLElement) {
		...
		const lastListItem = currentListItem.parentElement
			?.lastChild as HTMLElement;
		const lastSibling = lastListItem?.querySelector(
			'.fill-cipher-button'
		) as HTMLElement;
		lastSibling?.focus();
	}
	 ...
	 */
	private focusViewCipherButton(
		currentListItem: HTMLElement,
		currentButtonElement: HTMLElement
	) {
		const cipherContainer = currentListItem.querySelector(
			'.cipher-container'
		) as HTMLElement;
