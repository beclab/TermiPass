import { EVENTS, RedirectFocusDirection } from '../../../utils';
import {
...
class AutofillOverlayPageElement extends HTMLElement {
	...
	protected initOverlayPage(
		...
		globalThis.document.head.title = this.getTranslation(
	 ...
	 */
	protected postMessageToParent(
		message: AutofillOverlayPageElementWindowMessage
	) {
	...
	protected setupGlobalListeners(windowMessageHandlers: WindowMessageHandlers) {
		...
		globalThis.document.addEventListener(
			EVENTS.KEYDOWN,
			this.handleDocumentKeyDownEvent
		);
	}
