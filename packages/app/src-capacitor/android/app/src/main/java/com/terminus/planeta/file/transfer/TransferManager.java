...
import com.terminus.planeta.file.CameraSyncStatus;
...
import com.terminus.planeta.file.data.CameraSyncEvent;
...
public abstract class TransferManager {
    ...
    public static final String BROADCAST_ACTION = "com.termiPass.planeta.TX_BROADCAST";
     ...
     */
    protected List<TransferTask> transferringList = new ArrayList<>();
    /**
     * contains waiting tasks
     */
    protected List<TransferTask> waitingList = new ArrayList<>();
    ...
    public synchronized void doNext() {
        if (!waitingList.isEmpty()
            ...
            if (scanUploadStatus > 0) {
                VaultApp.Companion.getApplication().setCameraUploadNumber(cameraUploadWaitingNumber, cameraUploadTotalNumber);
                VaultApp.Companion.getApplication().setScanUploadStatus(CameraSyncStatus.UPLOADING);
                EventBus.getDefault().post(new CameraSyncEvent("upload"));
            }
            if (SettingsManager.instance().isFolderAutomaticBackup()) {
                VaultApp.Companion.getApplication().setFolderBackupNumber(folderBackupTotalNumber, folderBackupWaitingNumber);
            }
     ...
     *
     * @return true, if there are tasks whose {@link com.terminus.planeta.file.transfer.TaskState} is {@code TRANSFERRING}.
     *          false, otherwise.
     */
    ...
    public synchronized List<? extends TransferTaskInfo> getAllTaskInfoList() {
        ArrayList<TransferTaskInfo> infos = new ArrayList<>();
        Collection<TransferTask> values = allTaskList.values();
        for (TransferTask value : values) {
