...
public class BaseContainer : HandyJSON {
...
public class PBES2Container : BaseContainer {
    ...
    func _deriveAndSetKey(password: String) {
        ...
        let key = try? PKCS5.PBKDF2(password: password.bytes, salt: self.keyParams.salt.bytes, iterations: self.keyParams.iterations, keyLength: self.keyParams.keySize/8)
    ...
    func _unlock(password: String) throws -> Data? {
...
public class SharedContainer: BaseContainer {
