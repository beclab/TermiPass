			...
			if (node.kind === ts.SyntaxKind.CallExpression) {
				...
				if ((call.expression as ts.Identifier).escapedText === '$l') {
					...
					const { line, character } =
						file.getLineAndCharacterOfPosition(call.getFullStart());
					...
					if (original) {
						...
						if (comments && comments.length) {
							const rawComment = fileText.substring(
								comments[0].pos,
								comments[0].end
							);
							const matchComment = rawComment.match(
								/@tcomment: ([^\*\n]+)/
							);
...
`;
	...
	doc.contents = YAML.createNode(
		items.flatMap((item) => [item.original, item.translation])
	) as any;
...
export function toJSON(translation: Translation) {
	return JSON.stringify(
		translation.items.map(({ original, translation }) => [
			original,
			translation
		]),
		null,
...
export function fromJSON(str: string, language: string) {
	const items = JSON.parse(str).map(
		([original, translation]: [string, string]) => ({
			original,
			translation,
			sources: []
		})
...
export function merge(curr: Translation, prev: Translation) {
	for (const item of curr.items) {
		const prevItemIndex = prev.items.findIndex(
