...
// export const WebSocketURL = process.env.IS_PC_TEST
//   ? 'http://localhost:3190'
//   : 'http://localhost:3010';
// export const WebSocketSendURL = process.env.IS_PC_TEST
//   ? 'http://localhost:3190/callback/push'
//   : 'http://localhost:3010/websocket/message';
...
> {
  try {
    const response = await axios.get(
      'http://localhost:40010/tapr/ws/conn/list',
    );
...
export async function broadcastWebsocketMessage(payload: any): Promise<number> {
  try {
    ...
    await axios.post('http://localhost:40010/tapr/ws/conn/send', data);
...
export async function sendWebsocketMessage(
  ...
  try {
    ...
    const response = await axios.post(
      'http://localhost:40010/tapr/ws/conn/send',
      data,
    );
...
export async function closeWebsocketConnection(
  ...
  try {
    ...
    const response = await axios.post(
      'http://localhost:40010/tapr/ws/conn/close',
      data,
    );
